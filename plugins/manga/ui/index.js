/**
 * Mop Manga Plugin UI - Web Component Custom Element (<mop-plugin-manga>)
 ...
 */
class MopPluginManga extends HTMLElement {
  constructor() {
    super();
  ...
  updateTheme() {
    const root = this.shadowRoot.querySelector('.manga-plugin-container');
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .manga-plugin-container[data-theme="light"] {
        ...
        .manga-plugin-container[data-theme="light"] .header {
        ...
        .manga-plugin-container[data-theme="light"] p {
          color: #64748b;
        }
        .tab-bar {
          display: flex;
        ...
        .manga-plugin-container[data-theme="light"] .tab-bar {
          border-bottom-color: rgba(0, 0, 0, 0.08);
        }
        .tab-btn {
        ...
        .manga-plugin-container[data-theme="light"] .tab-btn {
          color: #64748b;
        }
        .tab-btn:hover {
          color: #f1f5f9;
        ...
        .manga-plugin-container[data-theme="light"] .tab-btn:hover {
          color: #0f172a;
        ...
        .manga-plugin-container[data-theme="light"] .tab-btn.active {
          color: #ffffff;
          background: #2563eb;
        }
        .tab-content {
        ...
        .manga-plugin-container[data-theme="light"] input[type="text"],
        .manga-plugin-container[data-theme="light"] input[type="password"] {
          background: #ffffff;
          border-color: #cbd5e1;
          color: #0f172a;
        }
        ...
        .manga-plugin-container[data-theme="light"] button.btn-secondary {
          background: #e2e8f0;
          border-color: #cbd5e1;
          color: #1e293b;
        }
        ...
        .manga-plugin-container[data-theme="light"] .output-box {
          background: #f8fafc;
          border-color: #e2e8f0;
          color: #0369a1;
        }
      ...
      <div class="manga-plugin-container" data-theme="${theme}">
        <div class="header">
          <div class="title-group">
            <span class="icon">📚</span>
            <div>
              <h2>Manga Plugin (mop.manga)</h2>
              <p>WebP CBZ Converter & Directory Watcher</p>
            </div>
          </div>
          <div>
            <button id="btn-run-doctor-header" class="btn-secondary">🩺 ヘルスチェック</button>
          </div>
        </div>

        <div class="tab-bar">
          <button class="tab-btn active" data-tab="convert" id="tab-convert">単一変換 (manga.convert)</button>
          <button class="tab-btn" data-tab="batch" id="tab-batch">一括変換 (manga.batch)</button>
          <button class="tab-btn" data-tab="inspect" id="tab-inspect">アーカイブ検査 (manga.inspect)</button>
          <button class="tab-btn" data-tab="doctor" id="tab-doctor">診断 (doctor)</button>
        ...
        <div class="tab-content active" id="pane-convert">
          <div class="form-group">
            <label for="convert-input">入力アーカイブパス (ZIP / CBZ / RAR / 7z / TAR / フォルダ):</label>
            <input type="text" id="convert-input" placeholder="/srv/incoming/sample.zip" />
          </div>
          <div class="form-group">
            <label for="convert-output">出力パス (任意、省略時は output_dir に配置):</label>
            <input type="text" id="convert-output" placeholder="/srv/output/sample.cbz" />
          </div>
          <div class="form-group">
            <label for="convert-password">アーカイブパスワード (暗号化時のみ):</label>
            <input type="password" id="convert-password" placeholder="パスワード" />
          </div>
          <div class="checkbox-group">
            <input type="checkbox" id="convert-dry-run" />
            <label for="convert-dry-run">Dry Run (ファイル生成を行わず検査のみ実施)</label>
          </div>
          <div class="actions-row">
            <button id="btn-convert-submit" class="btn-primary">変換ジョブ実行</button>
          </div>
        </div>
        ...
        <div class="tab-content" id="pane-batch">
          ...
          <div class="form-group">
            <label for="batch-password">共通アーカイブパスワード (任意):</label>
        ...
        <div class="tab-content" id="pane-inspect">
          <div class="form-group">
            <label for="inspect-input">検査アーカイブパス:</label>
            <input type="text" id="inspect-input" placeholder="/srv/incoming/sample.zip" />
          </div>
          <div class="form-group">
        ...
        <div class="tab-content" id="pane-doctor">
          <p>プラグイン環境依存ライブラリ (libarchive / libvips / ffmpeg) および設定レイアウトを診断します。</p>
          <div class="actions-row">
            <button id="btn-run-doctor" class="btn-primary">Doctor 診断を実行</button>
          </div>
        ...
        <div class="tab-content" id="pane-watcher">
          <p>常駐ウォッチャおよびプラグインメタデータを照会します。</p>
          <div class="actions-row">
  ...
  async submitConvertJob() {
    ...
    if (!input) {
      this.setOutput('エラー: 入力アーカイブパスを指定してください');
      return;
    }
  ...
  async submitBatchJob() {
    ...
    try {
      this.setOutput('manga.batch ジョブを送信中...');
      const res = await this._context.callRpc('job.submit', {
        job_type: 'manga.batch',
        params: { dir, password, dry_run: dryRun },
      });
      this.setOutput('ジョブ送信完了:\n' + JSON.stringify(res, null, 2));
      if (this._context.showNotification) {
        this._context.showNotification('success', 'manga.batch ジョブを送信しました');
      }
    } catch (e) {
      this.setOutput('ジョブ送信エラー:\n' + (e.message || String(e)));
    }
  ...
  async submitInspectJob() {
    ...
    if (!input) {
      this.setOutput('エラー: 検査対象アーカイブパスを指定してください');
      return;
    }
    const password = this.shadowRoot.getElementById('inspect-password')?.value || null;

    try {
      this.setOutput('manga.inspect ジョブを送信中...');
      const res = await this._context.callRpc('job.submit', {
        job_type: 'manga.inspect',
        params: { input, password },
      });
      this.setOutput('ジョブ送信完了:\n' + JSON.stringify(res, null, 2));
      if (this._context.showNotification) {
        this._context.showNotification('success', 'manga.inspect ジョブを送信しました');
      }
    } catch (e) {
      this.setOutput('ジョブ送信エラー:\n' + (e.message || String(e)));
    }
  ...
  async runDoctor() {
    ...
    try {
      this.setOutput('Doctor 診断を実行中...');
      const res = await this._context.callRpc('doctor', null);
      this.setOutput('Doctor 診断結果:\n' + JSON.stringify(res, null, 2));
...
if (!customElements.get('mop-plugin-manga')) {
  customElements.define('mop-plugin-manga', MopPluginManga);
}

export default MopPluginManga;
