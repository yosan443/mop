...
impl ResidentWatcher {
    pub fn start(cfg: Arc<MangaConfig>, pool: Arc<WorkerPool>) -> Result<Self, notify::Error> {
                    ...
                    _ = ticker.tick() => {
                        ...
                        for path in ready {
                            ...
                            if path.exists() && is_candidate_file(&path, &cfg_clone) {
                                debug!("watcher debounce ready, submitting: {}", path.display());
...
pub fn scan_existing(cfg: &MangaConfig, pool: &WorkerPool) {
    ...
    for dir in &cfg.watch_dirs {
            ...
            .filter_map(|e| e.ok())
        {
            ...
            if is_candidate_file(path, cfg) {
                info!("scan_on_start found: {}", path.display());
...
fn is_candidate_file(path: &Path, cfg: &MangaConfig) -> bool {
    if !path.is_file() {
        return false;
    }
    ...
    if path.starts_with(&cfg.output_dir)
        || path.starts_with(&cfg.unknown_dir)
        || path.starts_with(&cfg.work_dir)
    {
        return false;
    }
    true
